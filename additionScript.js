...
function hs(){
    if(localStorage.getItem('highScoreA')!=null && localStorage.getItem('highTimeA') != null)
    { document.getElementById('hs').innerHTML = 'High Score: ' + localStorage.getItem('highScoreA') + ' correct in ' + outputTime(localStorage.getItem('highTimeA'));}
...
async function pgame() {
    ...
    for (let i = 1; i < 11; i++) {
        let p = Math.floor(Math.random() * 250);
...
function retrieveInput(i,p,n,c) {
    return new Promise(resolve => {
        ...
        question.innerHTML = "What is " + p + " + " + n;
        ...
        submitButton.onclick = function () {
            ...
            if(flag % 2 == 0){
                ...
                if(i==10){
                    ...
                    if((correct >= localStorage.getItem('highScoreA') || (localStorage.getItem('highScoreA') == null))){
                        old_score = localStorage.getItem('highScoreA')
                        localStorage.setItem('highScoreA' , correct);
                        new_score = localStorage.getItem('highScoreA')

                        old_time = localStorage.getItem('highTimeA');
                        new_time = elapsedPausedTime;
                        if((new_time < old_time && old_score == new_score) || new_score > old_score || old_time == null) {
                            localStorage.setItem('highTimeA' , elapsedPausedTime);
                        }
                        new_time = localStorage.getItem('highTimeA');
...
function compareTimes() {
    if (elapsedPausedTime < localStorage.getItem('highTimeA')){
        return true;
    } else{
        return false;
    }
