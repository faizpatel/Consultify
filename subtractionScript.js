...
function hs(){
    if(localStorage.getItem('highScoreS')!=null && localStorage.getItem('highTimeS') != null)
    { document.getElementById('hs').innerHTML = 'High Score: ' + localStorage.getItem('highScoreS') + ' correct in ' + outputTime(localStorage.getItem('highTimeS'));}
        ...
        submitButton.onclick = function () {
            ...
            if(flag % 2 == 0){
                ...
                if(i==10){
                    ...
                    if((correct >= localStorage.getItem('highScoreS') || (localStorage.getItem('highScoreS') == null))){
                        old_score = localStorage.getItem('highScoreS')
                        localStorage.setItem('highScoreS' , correct);
                        new_score = localStorage.getItem('highScoreS')

                        old_time = localStorage.getItem('highTimeS');
                        new_time = elapsedPausedTime;
                        if((new_time < old_time && old_score == new_score) || new_score > old_score || old_time == null) {
                            localStorage.setItem('highTimeS' , elapsedPausedTime);
                        }
                        new_time = localStorage.getItem('highTimeS');
...
function compareTimes() {
    if (elapsedPausedTime < localStorage.getItem('highTimeS')){
        return true;
    } else{
        return false;
    }
